...
@min_players 10
 ...
 when length(players) == 2 do
  ...
  when length(players) < @max_players do
    ...
    ## check if players have enough stack for game if not kick them out of the game
     case player.stack < game.rules.small_blind do
       true ->  {:ok, %{game | players: game.players ++ [player]}}
       false ->  {:ok, %{game | players: game.players}}
     end
